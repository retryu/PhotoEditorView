 ...
 * 
 */
public abstract class ZoomableImageViewBase extends ImageView implements IDisposable {
	...
	/**
	 * Use this to change the {@link ZoomableImageViewBase#setDisplayType(DisplayType)} of 
	 * this View
	 * @author alessandro
	 *
	 */
	...
	public ZoomableImageViewBase( Context context, AttributeSet attrs ) {
		super( context, attrs );
		init();
	}
	...
	public void setScaleType( ScaleType scaleType ) {
		if ( scaleType == ScaleType.MATRIX ) {
			...
			ZebraLog.w( LOG_TAG, "Unsupported scaletype. Only MATRIX can be used" );
		}
	}
	...
	public void setDisplayType( DisplayType type ) {
		if ( type != mScaleType ) {
			if ( LOG_ENABLED ) {
				ZebraLog.i( LOG_TAG, "setDisplayType: " + type );
			}
			mUserScaled = false;
			mScaleType = type;
	...
	protected void setMinScale( float value ) {
		if ( LOG_ENABLED ) {
			ZebraLog.d( LOG_TAG, "setMinZoom: " + value );
		}
	...
	protected void setMaxScale( float value ) {
		if ( LOG_ENABLED ) {
			ZebraLog.d( LOG_TAG, "setMaxZoom: " + value );
		}
		mMaxZoom = value;
	}
	...
	protected void onLayout( boolean changed, int left, int top, int right, int bottom ) {
		...
		if ( LOG_ENABLED ) {
			ZebraLog.e( LOG_TAG, "onLayout: " + changed + ", bitmapChanged: " + mBitmapChanged + ", scaleChanged: " + mScaleTypeChanged );
		}
		...
		if ( drawable != null ) {
			...
			if ( changed || mScaleTypeChanged || mBitmapChanged ) {
				...
				if( LOG_ENABLED ) {
					ZebraLog.d( LOG_TAG, "old matrix scale: " + old_matrix_scale );
					ZebraLog.d( LOG_TAG, "new matrix scale: " + new_matrix_scale );
					ZebraLog.d( LOG_TAG, "old min scale: " + old_min_scale );
					ZebraLog.d( LOG_TAG, "old scale: " + old_scale );
				}
				...
				if ( mBitmapChanged || mScaleTypeChanged ) {
					...
					if( LOG_ENABLED ) {
						ZebraLog.d( LOG_TAG, "display type: " + mScaleType );
					}
					...
					if ( LOG_ENABLED ) {
						ZebraLog.d( LOG_TAG, "old min scale: " + old_default_scale );
						ZebraLog.d( LOG_TAG, "old scale: " + old_scale );
						ZebraLog.d( LOG_TAG, "new scale: " + scale );
					}
				...
				if( LOG_ENABLED ) {
					ZebraLog.d( LOG_TAG, "new scale: " + getScale() );
				}
			}
		} else {
			// drawable is null
	...
	protected void _setImageDrawable( final Drawable drawable, final Matrix initial_matrix, float min_zoom, float max_zoom ) {
		...
		if ( LOG_ENABLED ) {
			ZebraLog.i( LOG_TAG, "_setImageDrawable" );
		}

		if ( drawable != null ) {

			if ( LOG_ENABLED ) {
				ZebraLog.d( LOG_TAG, "size: " + drawable.getIntrinsicWidth() + "x" + drawable.getIntrinsicHeight() );
			}
	...
	protected void onDrawableChanged( final Drawable drawable ) {
		if( LOG_ENABLED ) {
			ZebraLog.i( LOG_TAG, "onDrawableChanged" );
		}
	...
	protected void onLayoutChanged( int left, int top, int right, int bottom ) {
		if( LOG_ENABLED ) {
			ZebraLog.i( LOG_TAG, "onLayoutChanged" );
		}
		ZebraLog.e("super.onLayoutChanged", "onLayoutChanged");
...
//		float scale = Math.max( fw, fh ) * 8;
		...
		if ( LOG_ENABLED ) {
			ZebraLog.i( LOG_TAG, "computeMaxZoom: " + scale );
		}
		return scale;
	}
	...
	protected float computeMinZoom() {
		...
		if ( LOG_ENABLED ) {
			ZebraLog.i( LOG_TAG, "computeMinZoom: " + scale );
		}
	...
	protected void getProperBaseMatrix( Drawable drawable, Matrix matrix ) {
		...
		if ( LOG_ENABLED ) {
			ZebraLog.d( LOG_TAG, "getProperBaseMatrix. view: " + viewWidth + "x" + viewHeight );
		}
	...
	public void printMatrix( Matrix matrix ) {
		...
		ZebraLog.d( LOG_TAG, "matrix: { x: " + tx + ", y: " + ty + ", scalex: " + scalex + ", scaley: " + scaley + " }" );
	}
	...
	protected void postTranslate( float deltaX, float deltaY ) {
		if ( deltaX != 0 || deltaY != 0 ) {
			if ( LOG_ENABLED ) {
				ZebraLog.i( LOG_TAG, "postTranslate: " + deltaX + "x" + deltaY );
			}
	...
	protected void postScale( float scale, float centerX, float centerY ) {
		if ( LOG_ENABLED ) {
			ZebraLog.i( LOG_TAG, "postScale: " + scale + ", center: " + centerX + "x" + centerY );
		}
		onScaleChange(scale);
